...
import { Landmark, AlertTriangle, CheckCircle, Flame, Plus, X } from 'lucide-react';
...
const Mines = () => {
  ...
  const [newMine, setNewMine] = useState({
    name: '', area: '', status: 'Operational', targetOutput: '', safetyStatus: 'Safe'
  });
  ...
  const handleCreateMine = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setNewMine({ name: '', area: '', status: 'Operational', targetOutput: '', safetyStatus: 'Safe' });
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mines.map((mine) => {
          ...
          return (
            <Card key={mine._id} className="flex flex-col justify-between space-y-4" hoverEffect>
